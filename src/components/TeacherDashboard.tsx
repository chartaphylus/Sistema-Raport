...
export function TeacherDashboard() {
  ...
  const [newTunggakan, setNewTunggakan] = useState({ nama: '', jumlah: 0 });
  ...
  const handleZipUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success > 0 && result.errors.length === 0) {
        setUploadMessage({
          type: 'success',
          message: `Berhasil memproses ${result.success} file raport!`
        });
  ...
  const handleAddTunggakan = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTunggakan.nama.trim() || newTunggakan.jumlah <= 0) return;

    setLoading(true);
    try {
      await addTunggakan(newTunggakan.nama.trim(), newTunggakan.jumlah);
      setNewTunggakan({ nama: '', jumlah: 0 });
      await loadTunggakan();
      setUploadMessage({
        type: 'success',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-green-100 p-4 sm:p-8">
          ...
          <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200">
            <h3 className="font-medium text-blue-900 mb-2">📝 Petunjuk Upload:</h3>
            <ul className="text-xs sm:text-sm text-blue-800 space-y-1">
              <li>• File harus berformat ZIP atau RAR</li>
              <li>• Berisi file PDF raport santri</li>
              <li>• Nama file PDF akan menjadi nama santri</li>
              <li>• Underscore (_) dan dash (-) akan diubah jadi spasi</li>
            </ul>
          </div>
        </div>
